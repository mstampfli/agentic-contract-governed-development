...
- Fix the class, not the instance: every instance in your files + a class test `tests_own/test_<your id>_class_*`
  ...
  (All roles "Sweep"; scope: every file of your modules and every spec entry they own or consume — not only what you
  changed): a class found → fix it the same way; a comment → fix it; a nit → fix or list it; a problem outside your
  files → list it in your reply; a spec gap → QUESTIONS.md / CHANGE REQUEST. Quality
  ...
  design).
- Reply: files, questions/assumptions, change requests, the sweep record (files and entries read in full; classes,
  comments and nits found — quality task: listed, not applied), stand-ins, disputed findings (why), the exact test command, line moves that stale others' citations.
...
- Sweep (All roles) before spawning any reviewer or red teamer, first rounds included: a spec or draft → the whole
  spec, every entry, plus the code implementing it; code you wrote (Mode 2) → every file of every module the change
  touches, calls or is called by, plus the whole spec; after a writer's run → its sweep record covers every file of its
  modules and ends with a whole pass that found nothing (else the run is not done). Record it in STATUS.md; a reviewer finding it missed → root cause in PROCESS.md.
...
## Scope map
| Component | Modules | Scope (now / planned / undecided; existing codebase: in scope / existing, unreviewed) | Depends on |
|---|---|---|---|
...
User scope override (existing codebase; the user's words, or "none"):
